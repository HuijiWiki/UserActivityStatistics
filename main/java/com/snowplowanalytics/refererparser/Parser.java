...
public class Parser {
  ...
  private Referer parse(String scheme, String host, String path, String query, String pageHost, List<String> internalDomains){
    ...
    //check if internal from source
    String[] splits = host.split("\\.");
	if(splits.length == 3 && 
	    ( splits[1].equals("huijiwiki") || ( splits[1].equals("huiji") && splits[2].equals("wiki") ))) 
		return new Referer(Medium.INTERNAL, "wiki", splits[0]) ;

    
    if (host.equals(pageHost)) return new Referer(Medium.INTERNAL, "wiki", "www");
    
    for (String s : internalDomains) {
      if (s.trim().equals(host))
        return new Referer(Medium.INTERNAL, "wiki", "www");
    }
    ...
    if (referer == null) {
      return new Referer(Medium.UNKNOWN, host, null); // Unknown referer, nothing more to do
    } else {
